...
Imports Hot.Checker.Commons
Imports NetOneRechargeClass
Imports NetOneRechargeClass.xDongleMessage

Public Class Commons
    Public Shared SQLConStr As String = "Data Source=127.0.0.1;Initial Catalog=HotChecker;Integrated Security=True"
...
Public Class xHotCheckerCheck
    ...
    Public ErrorCountThreshold As Integer = 2
    ...
    ReadOnly Property StatusData() As xHotCheckerCheckResult
        Get
            ...
            Dim nearhistory = Enumerable.Reverse(CheckResults).Take(5).ToList()
            Dim ErrorsInNearHistory = (From a As xHotCheckerCheckResult In nearhistory Where a.AlertLevel > xHotCheckerCheckResult.AlertLevels.LowPriority)
            Dim WarningsInNearHistory = (From a As xHotCheckerCheckResult In nearhistory Where a.AlertLevel = xHotCheckerCheckResult.AlertLevels.LowPriority)
            ...
            If Errors > 0 Then HighestErrorLevel = (From e As xHotCheckerCheckResult In ErrorsInNearHistory Order By e.AlertLevel Descending Select e Take 1)(0).AlertLevel
    ...
    Public ReadOnly Property LogItem As xHotCheckerLogItem
        Get
            Return New xHotCheckerLogItem(Me.ErrorLogCheckID, ErrorCount, StatusString)
        End Get
    End Property


    Sub New(Optional _configName As String = "New Config", Optional _CheckPing As Boolean = False, Optional _IPAdd As String = "127.0.0.1",
               Optional _CheckLatency As Boolean = False, Optional _Latency As Long = 500, Optional _CheckURL As Boolean = False,
               Optional _URL As String = "", Optional _CheckContent As Boolean = False, Optional _ExpectedContent As String = "")
    ...
    End Sub
    Sub New(spRead As SqlDataReader)
        read(spRead)

    End Sub
    Sub read(spRead As SqlDataReader)
        ...
        ErrorCountThreshold = IIf(IsDBNull(spRead.Item("ErrorCount")), 0, spRead.Item("ErrorCount"))
    ...
    Function RunThisCheck() As Boolean
        ...
        If result.AlertLevel > xHotCheckerCheckResult.AlertLevels.LowPriority Then
            If ErrorCount >= ErrorCountThreshold Then
                SuccessCount = 0
        ...
        Return True
    End Function
    Private Sub IntervalTimer_Tick(sender As Object, e As EventArgs) Handles IntervalTimer.Elapsed
...
Public Class xHotCheckerLogItem
    ...
    End Function
       ...
       Public Shared Function List(ByVal BankTrxBatchID As Long, ByVal sqlConn As SqlConnection, Optional ByVal sqlTrans As SqlTransaction = Nothing) As List(Of xBankTrx)
        Dim iList As New List(Of xHotCheckerLogItem)
        Using sqlCmd As New SqlCommand("xBankTrx_List", sqlConn, sqlTrans)
            ...
            Using sqlRdr As SqlDataReader = sqlCmd.ExecuteReader
                While sqlRdr.Read
                    iList.Add(New xHotCheckerLogItem(sqlRdr))

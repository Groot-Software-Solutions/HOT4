...
jQuery.validator.addMethod('phonesZim', function(phone_number, element) {
	phone_number = phone_number.replace(/\(|\)|\s+|-/g,'');
	return this.optional(element) || phone_number.length > 9 &&
		phone_number.match(/^(?:(?:(?:00\s?|\+)263\s?|0)(7(?:[1378]\d{2})\s?\d{3}\s?\d{2}|86\d{2}\s?\d{3}\s?\d{3}))$/);
}, 'Please specify a valid mobile number');

jQuery.validator.addMethod('isEmailRegistered', function (value, element) {
    var result = false;
   // console.log(element);
    if (jQuery(element).hasClass("checked-email") ){
        if (jQuery(element).hasClass("registered")) {
            result = false;
        } else { 
            result = true; 
        };

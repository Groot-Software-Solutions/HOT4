...
Public Class xSMS
    ...
    Sub Fill(ByVal sqlRdr As SqlDataReader)
        _SMSID = sqlRdr("SMSID")
...
Public Class xSMSAdapter
    Public Shared Sub Save(ByVal iSMS As xSMS, ByVal sqlConn As SqlConnection)
        Using sqlCmd As New SqlCommand("xSMS_Save", sqlConn)
    ...
    '        ByVal ProcessStateID As Integer, ByVal SmppID As Guid, ByVal sqlConn As SqlConnection, Optional ByVal sqlTrans As SqlTransaction = Nothing) As List(Of xSMS)
    '    Dim iList As New List(Of xSMS)
    ...
    '        sqlCmd.Parameters.AddWithValue("SmppID", IIf(SmppID = Guid.Empty, Nothing, SmppID))
    '        sqlCmd.Parameters.AddWithValue("ProcessStateID", ProcessStateID)
    '        Using sqlRdr As SqlDataReader = sqlCmd.ExecuteReader
    '            While sqlRdr.Read
    '                iList.Add(New xSMS(sqlRdr))
    '            End While

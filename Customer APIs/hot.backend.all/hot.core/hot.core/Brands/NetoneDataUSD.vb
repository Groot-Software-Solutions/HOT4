...
Namespace Brands
    Public Class NetoneDataUSD
        ...
        Private Function CreateCreditRequest(recharge As xRecharge, iRechargePrepaid As xRechargePrepaid,
                                         numberOfDays As Integer) As RechargeUSDBundleRequest
            Return New RechargeUSDBundleRequest With {
            .Body = New RechargeUSDBundleRequestBody With {
        ...
        Private Sub CreditSubscriber(recharge As xRecharge, rechargePrepaid As xRechargePrepaid, numberOfDays As Integer)
            Dim creditRequest As RechargeUSDBundleRequest = CreateCreditRequest(recharge, rechargePrepaid, numberOfDays)
            Dim client = GetNetworkClient()
            Try
                Console.WriteLine("Crediting")
                Dim bundleClient = client.GetNetwork()
                Dim response = bundleClient.RechargeUSDBundle(creditRequest).Body.RechargeUSDBundleResult
                ...
                rechargePrepaid.InitialWallet = response.WalletBalance + IIf(recharge.IsSuccessFul, response.Amount, 0)

...
namespace Hot4.Repository.Concrete
{
    public class StatisticsRepository : IStatisticsRepository
    {
        ...
        {
            return await _context.Recharge.Include(d => d.Brand).ThenInclude(d => d.Network)
                 .Where(d => d.StateId == (int)SmsState.Success && d.RechargeDate > DateTime.Now.AddMinutes(-11))
                 .GroupBy(d => new
                 {
                 ...
                 .OrderBy(d => d.Name).ThenByDescending(d => d.X)
                 .ToListAsync();
        }
        public async Task<List<StatisticsTrafficModel>> GetRechargeTrafficDayStatistics()
        {
            return await _context.Recharge.Include(d => d.Brand).ThenInclude(d => d.Network)
                 .Where(d => d.StateId == (int)SmsState.Success && d.RechargeDate > DateTime.Now.AddHours(-24))
                 .GroupBy(d => new
                 {
        ...
        {
            return await _context.Sms.Include(d => d.State).Where(d =>
            d.Direction == true && d.Smsdate > DateTime.Now.AddMinutes(-11))
               ...
               .OrderBy(d => d.Name).ThenBy(d => d.Name)
               .ToListAsync();
        }
    }
}
